...
criteria live in `SPEC.md` (accepted 2026-09-03); where the two differ, the
...
> File paths such as `core/tg_bot.py`, `core/db.py:23` or `tests/test_intake.py` refer to the
> **JobScraper** repository (`Dmitreyvladimirov/job-scraper`), whose patterns are copied
> here. Nothing is shared at runtime.
...
`SPEC.md` §0 ("Takeaways from Вкалывают роботы").
...
## 4. Architecture (as of 0.1; the current one is in `SPEC.md` §7)
...
## 7. Work queue (as of 0.1; the accepted stages are in `SPEC.md` §11)
...
All of these were closed on 09-01…09-03; the answers are in `SPEC.md` §0 and

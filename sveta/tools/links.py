...
def save(scope: UserScope, ctx: ToolContext, url: str, comment: str = "") -> tuple[str, int | None, "fetch.FetchResult | None"]:
    """(message for the model, note_id or None, the fetch result or None)."""
    url, comment = (url or "").strip(), (comment or "").strip()
    if not url.lower().startswith(("http://", "https://")):
        return "Error: url must start with http:// or https://.", None, None
    ...
    if not result.ok:
        ...
        return f"Link recorded but NOT saved as a note: could not fetch it ({why}). Tell the user.", None, result
    ...
    return f"Saved note #{note_id}: {result.title or url}{summary}", note_id, result
